...
import { CustomizationConfig, CustomizationOption, CustomizationSelection, PriceBreakdown, CustomizationSummaryItem } from '../../core/models/customization.model';
...
@Component({
  ...
  template: `
    @if (isOpen && product) {
      <!-- Overlay -->
      <div class="customization-overlay" (click)="onClose()"></div>
      ...
      <div class="customization-modal">
        <div class="customization-modal-content">
          ...
          <div class="customization-header">
            ...
            <button class="close-customization" (click)="onClose()" aria-label="Close">&times;</button>
          </div>
          ...
          <div class="stepper-container">
            <div class="stepper-horizontal">
              @for (step of steps; track step.number; let i = $index) {
                <div class="stepper-step" [class.active]="currentStep === step.number" [class.completed]="currentStep > step.number">
          ...
          <div class="customization-body overflow-y-auto flex-1 p-8">
            ...
            @if (currentStep === 1 && config) {
              <div class="slide-in">
                <h3 class="text-2xl font-bold text-gray-800 mb-2">Select Package Size</h3>
                  ...
                  @for (value of getOptionValues('package_size'); track value.value) {
                    <div
                      ...
                      <div class="option-card-header">
                        ...
                        <span class="option-price" [class.free]="value.price_modifier === 0">
                          {{ customizationService.formatPriceModifier(value.price_modifier) }}
                        </span>
                      </div>
            ...
            @if (currentStep === 2 && config) {
              <div class="slide-in">
                @if (product.category === 'black') {
                  <h3 class="text-2xl font-bold text-gray-800 mb-2">Brew Preferences</h3>
                ...
                @if (product.category === 'green') {
                  <h3 class="text-2xl font-bold text-gray-800 mb-2">Leaf Style & Accessories</h3>
                    ...
                    <select
                      ...
                      @for (value of getOptionValues('leaf_style'); track value.value) {
                        <option [value]="value.value">
                          {{ value.display_name }} {{ value.price_modifier > 0 ? '(+$' + value.price_modifier + ')' : '' }}
                        </option>
                      }
                    </select>
                  </div>
                ...
                @if (product.category === 'oolong') {
                  <h3 class="text-2xl font-bold text-gray-800 mb-2">Roast Level & Brewing</h3>
                  <p class="text-gray-600 mb-6">Select roast and optional brewing vessel</p>
                  ...
                  <div class="form-group">
                    <label class="form-label">Brewing Vessel</label>
                ...
                @if (product.category === 'herbal') {
                  ...
                  <p class="text-gray-600 mb-6">Select blend and add extras (up to 3)</p>
                  ...
                  <div class="form-group">
                    <label class="form-label">Extras ({{ getSelectedExtras().length }}/3)</label>
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
                      @for (value of getOptionValues('extras'); track value.value) {
                        <div
                          class="option-card"
                          [class.selected]="isExtraSelected(value.value)"
                          [class.opacity-50]="!isExtraSelected(value.value) && getSelectedExtras().length >= 3"
                          (click)="toggleExtra(value.value, value.price_modifier)">
                          <div class="option-card-header">
                            <span class="option-name">{{ value.display_name }}</span>
                            <span class="option-price">{{ customizationService.formatPriceModifier(value.price_modifier) }}</span>
                          </div>
                          <div class="selected-indicator">✓</div>
                        </div>
                      }
            ...
            @if (currentStep === 4 && config) {
              <div class="slide-in">
                ...
                <p class="text-gray-600 mb-6">Please review your selections before adding to cart</p>
                    ...
                    <div class="space-y-3">
                      @for (item of summary; track item.label) {
                        <div class="flex justify-between items-center py-2 border-b border-gray-100">
                          ...
                          @if (item.price > 0) {
                            <span class="text-gold font-semibold">{{ customizationService.formatPriceModifier(item.price) }}</span>
                          }
                        </div>
                      }
                    </div>
                ...
                <div class="price-summary">
                  ...
                  @if (priceInfo && priceInfo.customizationCost > 0) {
                    <div class="price-breakdown-row total">
                      <span>Total</span>
                      <span>{{ customizationService.formatPrice(priceInfo.totalPrice) }}</span>
                    </div>
                  }
                </div>
              </div>
            ...
            <div class="footer-left">
              @if (priceInfo) {
                <div class="text-right">
                  ...
                  <div class="text-2xl font-bold text-gold">{{ customizationService.formatPrice(priceInfo.totalPrice) }}</div>
                </div>
              }
            </div>
...
export class CustomizationModalComponent implements OnChanges {
  ...
  getOptionValues(optionId: string): any[] {
  ...
  private updatePrice(): void {
    ...
    this.summary = this.customizationService.formatSummary(this.customizations, this.config);
  }
  ...
  nextStep(): void {
    ...
    const validation = this.customizationService.validateStep(this.currentStep, this.customizations, this.config);
  ...
  private completeCustomization(): void {
    ...
    const customizedItem: CartItem = {
      ...this.product,
      id: this.customizationService.generateCustomizationId(this.product.id) as any,
      isCustomized: true,
