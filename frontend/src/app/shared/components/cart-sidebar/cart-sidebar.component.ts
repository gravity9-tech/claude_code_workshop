...
@Component({
  ...
  template: `
    ...
    <div
      ...
      [class.translate-x-full]="!(cartService.isOpen$ | async)"
      ...
      <div class="h-full flex flex-col">
            ...
            @for (item of cartService.items$ | async; track item.id) {
              <div class="cart-item bg-white rounded-lg shadow p-4 mb-4">
                @if (item.isCustomized) {
                  <div class="inline-block bg-gold text-white text-xs px-2 py-1 rounded mb-2">Customized</div>
                }
        ...
        <div class="border-t p-6 bg-gray-50">
          <div class="flex justify-between items-center mb-4">
            ...
            <span class="text-2xl font-bold text-gold">\${{ cartService.getTotal().toFixed(2) }}</span>
          </div>
          <button class="w-full bg-gold hover:bg-dark-gold text-white font-bold py-3 rounded-lg transition-colors">
            Checkout
          </button>
        </div>
      </div>
