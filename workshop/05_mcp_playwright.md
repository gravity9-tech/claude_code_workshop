...
```bash
claude mcp add playwright --scope project -- npx -y @anthropic-ai/mcp-server-playwright
```
...
- `npx -y @anthropic-ai/mcp-server-playwright` - The server command
...
```bash
# Check if npx works
npx -y @anthropic-ai/mcp-server-playwright --help
```
...
You've now learned all the core Claude Code features:

| Module | Feature | Purpose |
|--------|---------|---------|
| 01 | `/init` | Project memory |
| 02 | Commands | Automation |
| 03 | Skills | Expertise |
| 04 | Subagents | Parallel work |
| 05 | MCP | External tools |
